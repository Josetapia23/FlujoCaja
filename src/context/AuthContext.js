...
export const AuthProvider = ({children}) => {
  ...
  const register = (nombre, edad, email, contrasena) => {
    return new Promise((resolve, reject) => {
      ...
      axios
        .post(
          'http://192.168.1.116/backFlujoCaja/registro.php',
          {
            nombre,
            edad,
            email,
        ...
        .then(res => {
          ...
          if (userInfo.result === 'success') {
            ...
            //http://192.168.1.116/backFlujoCaja/login.php
          }
  ...
  const updateUserInfo = async () => {
    try {
      const response = await axios.post(
        'http://192.168.1.116/backFlujoCaja/getUserInfo.php',
        {
          idUser: userInfo.id,
        }
      );
  ...
  const login = (email, contrasena) => {
    return new Promise((resolve, reject) => {
      setIsLoading(true);
      axios
        .post('http://192.168.1.116/backFlujoCaja/login.php', {
          email,
          contrasena,
        })
        .then(res => {
  ...
  const obtenerEmpresa = async () => {
    try {
      ...
      const respuesta = await axios.post(
        'http://192.168.1.116/backFlujoCaja/getDatosEmpresa.php',
        { id: userInfo.id }
      );
  ...
  const montos = async () => {
    try {
      setIsLoading(true);
      const respuesta = await axios.post(
        'http://192.168.1.116/backFlujoCaja/montosPorHora.php',
        {
          idUser: userInfo.id,
  ...
  const registerEmpresa = (nombreEmpresa, nit, direccion, telefonoEmpresarial, emailEmpresarial, idUser, idDepartamento, idMunicipio, registroEmpresa) => {
    return new Promise((resolve, reject) => {
      ...
      axios
        .post(
          'http://192.168.1.116/backFlujoCaja/registEmpresa.php',
          {
            nombreEmpresa,
            nit,
            direccion,
